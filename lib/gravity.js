var cmd = require('./cmd');	
var channel = require('./clientChannel');	
...
function aurora(ip, port) {
  ...
  this.clientChannel = new channel(this.ip, this.port);
...
aurora.prototype.clearPixel = function (callback){
	...
	self.clientChannel.connect(function(){
    self.clientChannel.send(buf, callback);
	});	
};	
